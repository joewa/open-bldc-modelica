...
block DetectCommutationZeroCrossing
  ...
  Modelica.Blocks.Continuous.FirstOrder filtV(T=1e-6)
...
equation
  ...
  when not senseBEMF then
    reinit(time2commutate, 1);// hier richtige Zeit
    reinit(time_zc, 1);
    reinit(maxBEMF, 0);
  elsewhen maxBEMF < abs(v[senseBridgeID] - v_dc / 2) then
    reinit(maxBEMF,  abs(v[senseBridgeID] - v_dc / 2));
  elsewhen ((pre(v_greaterthan0) == true) and (v_greaterthan0 == false)) then // detect negative zero crossing
    reinit(time_zc, time);
    reinit(time2commutate, sqrt(abs(-KV/dv))/6 - DelayCommutation); // hier noch dir "richtige" Zeit bestimmen und die 0.00006 ersetzen!
  end when;
