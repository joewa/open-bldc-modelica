...
block TestCtrl3phPWM "Commutation applying PWM"
  ...
  Modelica.Blocks.Sources.Constant const2[3](each k = 0.0001) annotation(Placement(transformation(extent = {{-90, 48}, {-70, 68}})));
  Modelica.Blocks.Sources.IntegerExpression integerExpression[3](each y = 1) annotation(Placement(transformation(extent = {{-42, -56}, {-22, -36}})));
...
equation
  ...
  connect(pulseWidth.y, halfBridgeLogic.pulses) annotation(Line(points = {{-9, 84}, {6, 84}, {6, 7.8}, {20, 7.8}}, color = {255, 0, 255}, smooth = Smooth.None));
  connect(integerExpression.y, halfBridgeLogic.pwm_Mode) annotation(Line(points = {{-21, -46}, {0, -46}, {0, -8}, {20, -8}}, color = {255, 127, 0}, smooth = Smooth.None));
