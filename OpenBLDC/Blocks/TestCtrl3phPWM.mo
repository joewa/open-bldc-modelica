...
block TestCtrl3phPWM "Commutation applying PWM"
  ...
  PulseWidth pulseWidth annotation(Placement(transformation(extent = {{-20,70},{0,90}})));
  Modelica.Blocks.Sources.Constant const(k = 0.9) annotation(Placement(transformation(extent = {{-78,78},{-58,98}})));
  Modelica.Blocks.Sources.Constant const1(k = 0) annotation(Placement(transformation(extent = {{-74,32},{-54,52}})));
...
equation
  ...
  connect(halfBridgeCtrl_c.hCtrl,hCtrl[3]) annotation(Line(points = {{60,-54},{76,-54},{76,66.6667},{100,66.6667}}, color = {255,0,255}, smooth = Smooth.None));
  connect(halfBridgeCtrl_a.lCtrl,lCtrl[1]) annotation(Line(points = {{60,44},{84,44},{84,-66.6667},{100,-66.6667}}, color = {255,0,255}, smooth = Smooth.None));
  ...
  connect(const.y,pulseWidth.d) annotation(Line(points = {{-57,88},{-38,88},{-38,84},{-20,84}}, color = {0,0,127}, smooth = Smooth.None));
  connect(const1.y,pulseWidth.startTime) annotation(Line(points = {{-53,42},{-36,42},{-36,76},{-20,76}}, color = {0,0,127}, smooth = Smooth.None));
  connect(const2.y,pulseWidth.T) annotation(Line(points = {{-67,62},{-44,62},{-44,80},{-20,80}}, color = {0,0,127}, smooth = Smooth.None));
