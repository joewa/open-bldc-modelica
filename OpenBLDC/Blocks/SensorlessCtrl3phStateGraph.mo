...
block SensorlessCtrl3phStateGraph "Commutation applying PWM"
  ...
  Modelica.Blocks.Tables.CombiTable1Ds combiTable1Ds(table=[0,1,0,0; 1,1,-1,0;
        2,0,-1,1; 3,-1,0,1; 4,-1,1,0; 5,0,1,-1; 6,1,0,-1])                                                            annotation(Placement(transformation(extent={{-48,14},
            {-28,34}})));
  PulseWidthVar pulseWidth[3] annotation(Placement(transformation(extent={{-30,-82},
            {-10,-62}})));
  ...
  Modelica.Blocks.Sources.BooleanExpression booleanExpression[3](each y = time > Modelica.Constants.small) annotation(Placement(transformation(extent={{-60,
            -100},{-40,-80}})));
  HalfBridgeDriver halfBridgeDriver[3] annotation(Placement(transformation(extent={{194,8},
            {214,28}})));
  HalfBridgeLogic halfBridgeLogic[3] annotation(Placement(transformation(extent={{166,8},
            {186,28}})));
  ...
  Modelica.StateGraph.InitialStep initialStep annotation(Placement(transformation(extent={{-54,-24},
            {-34,-4}})));
  Modelica.StateGraph.StepWithSignal senseBEMF annotation(Placement(transformation(extent={{40,-24},
            {60,-4}})));
  ...
  Modelica.Blocks.Sources.BooleanExpression booleanExpression1(each y = chAngle) annotation(Placement(transformation(extent={{-44,-46},
            {-24,-26}})));
  Modelica.StateGraph.TransitionWithSignal transitionWithSignal1(enableTimer=
        false)                                                   annotation(Placement(transformation(extent={{-30,-24},
            {-10,-4}})));
  ...
  Modelica.StateGraph.Step wait
    annotation (Placement(transformation(extent={{-12,-24},{8,-4}})));
equation
  intAngle = integer(angle);
  chAngle = change(intAngle);
  connect(angle,combiTable1Ds.u) annotation(Line(points={{-100,0},{-74,0},{-74,
          24},{-50,24}},                                                                      color = {0,0,127}, smooth = Smooth.None));
  connect(const2.y,pulseWidth.periodTime) annotation(Line(points={{-69,-72},{
          -30,-72}},                                                                                      color = {0,0,127}, smooth = Smooth.None));
  connect(booleanExpression.y,pulseWidth.active) annotation(Line(points={{-39,-90},
          {-36,-90},{-36,-80},{-30,-80}},                                                                        color = {255,0,255}, smooth = Smooth.None));
  connect(halfBridgeLogic.pin_IN,halfBridgeDriver.pin_IN) annotation(Line(points={{186,24},
          {194,24}},                                                                                  color = {255,0,255}, smooth = Smooth.None));
  connect(halfBridgeLogic.pin_SD,halfBridgeDriver.pin_SD) annotation(Line(points={{186,12},
          {194,12}},                                                                                    color = {255,0,255}, smooth = Smooth.None));
  connect(halfBridgeDriver.lCtrl,lCtrl) annotation(Line(points={{214,12},{220,
          12},{220,-60},{240,-60}},                                                                       color = {255,0,255}, smooth = Smooth.None));
  connect(halfBridgeDriver.hCtrl,hCtrl) annotation(Line(points={{214,24},{220,
          24},{220,60},{240,60}},                                                                     color = {255,0,255}, smooth = Smooth.None));
  connect(combiTable1Ds.y,halfBridgeLogic.val) annotation(Line(points={{-27,24},
          {122,24},{122,18},{166,18}},                                                                   color = {0,0,127}, smooth = Smooth.None));
  connect(pulseWidth.y,halfBridgeLogic.pulses) annotation(Line(points={{-10,-72},
          {158,-72},{158,25.8},{166,25.8}},                                                                      color = {255,0,255}, smooth = Smooth.None));
  connect(integerExpression.y,halfBridgeLogic.pwm_Mode) annotation(Line(points={{143,-6},
          {154,-6},{154,10},{166,10}},                                                                                    color = {255,127,0}, smooth = Smooth.None));
  connect(senseBEMF.outPort[1],transitionWithSignal.inPort) annotation(Line(points={{60.5,
          -14},{70,-14}},                                                                                   color = {0,0,0}, smooth = Smooth.None));
  connect(detectCommutationSimple.senseBEMF,senseBEMF.active) annotation(Line(points={{50,-32},
          {50,-25}},                                                                                        color = {255,0,255}, smooth = Smooth.None));
  connect(detectCommutationSimple.pulses,pulseWidth.y) annotation(Line(points={{40,-38},
          {34,-38},{34,-72},{-10,-72}},                                                                               color = {255,0,255}, smooth = Smooth.None));
  connect(detectCommutationSimple.bridgeState,combiTable1Ds.y) annotation(Line(points={{40,-46},
          {28,-46},{28,24},{-27,24}},                                                                                       color = {0,0,127}, smooth = Smooth.None));
  connect(detectCommutationSimple.v_dc,v_dc) annotation(Line(points={{54,-52},{
  ...
  connect(booleanExpression1.y,transitionWithSignal1.condition) annotation(Line(points={{-23,-36},
          {-20,-36},{-20,-26}},                                                                                           color = {255,0,255}, smooth = Smooth.None));
  connect(transitionWithSignal.outPort, initialStep.inPort[1]) annotation (Line(
      points={{75.5,-14},{92,-14},{92,6},{-60,6},{-60,-14},{-55,-14}},
      color={0,0,0},
      smooth=Smooth.None));
  connect(pulseWidth[1].dutyCycle, dutyCycle) annotation (Line(
      points={{-30,-64},{-66,-64},{-66,80},{-100,80}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(dutyCycle, pulseWidth[2].dutyCycle) annotation (Line(
      points={{-100,80},{-66,80},{-66,-64},{-30,-64}},
      color={0,0,127},
      smooth=Smooth.None));
  ...
  connect(delaySense.outPort, senseBEMF.inPort[1]) annotation (Line(
      points={{19.5,-14},{39,-14}},
      color={0,0,0},
      smooth=Smooth.None));
  connect(transitionWithSignal1.outPort, wait.inPort[1]) annotation (Line(
      points={{-18.5,-14},{-13,-14}},
      color={0,0,0},
      smooth=Smooth.None));
  ...
  annotation(Diagram(coordinateSystem(preserveAspectRatio=false,   extent={{-100,
            -100},{240,100}}),                                                                        graphics), Icon(coordinateSystem(extent = {{-100,-100},{240,100}}, preserveAspectRatio = false), graphics));
