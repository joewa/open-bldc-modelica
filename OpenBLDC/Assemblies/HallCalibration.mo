...
model HallCalibration "Constant speed demonstrate hall sensor commutation"
  ...
  Machines.PMSM pMSM_ab(R_p = 0.33 / 2, L_p = 0.000035 / 2, PhaseBEMF = 0.1) annotation(Placement(transformation(extent = {{-50, 74}, {-30, 94}})));
  ...
  Machines.PMSM pMSM_bc(R_p = 0.33 / 2, L_p = 0.000035 / 2, PhaseBEMF = 0.1) annotation(Placement(transformation(extent = {{-50, 46}, {-30, 66}})));
  Modelica.Electrical.Analog.Basic.Ground ground1 annotation(Placement(transformation(extent = {{-92, -10}, {-72, 10}})));
  Machines.PMSM pMSM_ac(R_p = 0.33 / 2, L_p = 0.000035 / 2, PhaseBEMF = 0.1) annotation(Placement(transformation(extent = {{-50, 18}, {-30, 38}})));
  Machines.PMSM pMSM_run(R_p = 0.33 / 2, L_p = 0.000035 / 2, PhaseBEMF = 0.1) annotation(Placement(transformation(extent = {{-50, -80}, {-30, -60}})));
...
equation
  ...
  connect(pMSM_ab.a2, pMSM_ab.b2) annotation(Line(points = {{-50, 90.4}, {-58, 90.4}, {-58, 82.4}, {-50, 82.4}}, color = {0, 0, 255}, smooth = Smooth.None));
  connect(pMSM_ab.c2, pMSM_ab.b2) annotation(Line(points = {{-50, 74.4}, {-58, 74.4}, {-58, 82.4}, {-50, 82.4}}, color = {0, 0, 255}, smooth = Smooth.None));
  connect(pMSM_ab.a1, pMSM_ab.b1) annotation(Line(points = {{-50, 93.6}, {-66, 93.6}, {-66, 85.6}, {-50, 85.6}}, color = {0, 0, 255}, smooth = Smooth.None));
  ...
  connect(pMSM_ac.support, fixed.flange) annotation(Line(points = {{-30, 20}, {-10, 20}, {-10, -86}}, color = {0, 0, 0}, smooth = Smooth.None));
  connect(pMSM_bc.support, fixed.flange) annotation(Line(points = {{-30, 48}, {-10, 48}, {-10, -86}}, color = {0, 0, 0}, smooth = Smooth.None));
  connect(ground1.p, pMSM_ab.a1) annotation(Line(points = {{-82, 10}, {-82, 93.6}, {-50, 93.6}}, color = {0, 0, 255}, smooth = Smooth.None));
